...
void print_section_headers(int32_t fd, int32_t fdOut ,Elf32_Ehdr eh, Elf32_Shdr sh_table[])
{
	uint32_t i;
	...
	for(i=0; i<eh.e_shnum; i++) {
		...
		else if(strcmp((sh_str + sh_table[i].sh_name),".bss") == 0){
		...
		else if(strcmp((sh_str + sh_table[i].sh_name),".data") == 0){
...
int32_t main(int32_t argc, char *argv[])
{
	...
	if(argc!=3) {
		printf("Usage: elf-parser <ELF-file> <noff-file>\n");
		return 0;
	}
	...
	print_section_headers(fd, fdOut, eh, sh_tbl);
